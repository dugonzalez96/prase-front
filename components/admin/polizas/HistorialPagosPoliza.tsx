...
export const HistorialPagosPoliza = ({
  numeroPoliza,
}: PropiedadesHistorialPagos) => {
  ...
  const calcularTotalPorPago = (
    pagosRealizados: { montoPagado: number }[]
  ): number => {
    return pagosRealizados.reduce((total, pago) => total + pago.montoPagado, 0);
  };

  const formatearFecha = (
    fechaUTC: string | Date | null | undefined
  ): string => {
    if (!fechaUTC) return "Fecha no disponible"; // ✅ Si es null/undefined, devolver mensaje genérico

    const fecha = new Date(fechaUTC);
    if (isNaN(fecha.getTime())) return "Fecha inválida"; // ✅ Si la fecha es inválida, devolver mensaje genérico
        ...
        <div className="space-y-6">
              ...
              <ScrollArea className="h-[300px]">
                <div className="space-y-4">
                  {esquemaPago?.esquemaPagos.map((pago, index) => {
                    const totalPagado = calcularTotalPorPago(
          ...
          {esquemaPago?.mensajeAtraso && (
            <Alert variant="destructive">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>{esquemaPago.mensajeAtraso}</AlertDescription>
            </Alert>
          )}
        </div>
      )}
