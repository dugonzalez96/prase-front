...
export const ActivarPolizaForm = ({
  ...
  const subirDocumentos = async (archivos: ArchivosBase64) => {
    ...
    try {
      const promesas = Object.entries(archivos).map(
        async ([nombre, base64]) => {
          if (!base64) return;
  ...
  const manejarConfirmacion = async (datosResumen: {
    ...
    const numeroPagos =
      tiposPago.find((tipo) => tipo.TipoPagoID === datosResumen.tipoPagoID)
        ?.Divisor ?? 12;
  ...
  if (isPending) {
    return <LoaderModales texto="Activando Póliza..." />;
  }
      ...
      <div className="space-y-6">
        ...
        <Dialog open={modalDocumentosAbierto} onOpenChange={manejarCierreModal}>
          <DialogContent className="max-w-4xl max-h-[600px] 2xl:max-h-[1200px] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Subir documentos del cliente</DialogTitle>
            </DialogHeader>
