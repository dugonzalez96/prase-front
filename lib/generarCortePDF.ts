...
export const generarCortePDF = async (data: any) => {
    ...
    const empleadoNombre = corte.usuarioID?.NombreUsuario || "Desconocido";
    const fechaCorte = format(new Date(corte.FechaCorte), "d 'de' MMMM 'del' yyyy", { locale: es })
    ...
    try {
        ...
        doc.addImage(logoBase64, "PNG", pageWidth - logoSize - 14, 10, logoSize, logoSize);
    } catch (err) {
    ...
    // Título
    doc.setFontSize(16);
    doc.text("Corte de Caja", pageWidth / 2, 20, { align: "center" });
    ...
    if (ingresos.length) {
        ...
        startY = addTableSection("Ingresos", [["Monto", "Forma de Pago", "Fecha", "Descripción"]], body, startY);
    }
    ...
    if (egresos.length) {
        ...
        startY = addTableSection("Egresos", [["Monto", "Forma de Pago", "Fecha", "Descripción"]], body, startY);
    }
    ...
    const firmaLabel = "Firma del responsable";
